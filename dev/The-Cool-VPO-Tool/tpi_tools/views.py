...
queue_BinDefs_old_list = [] ##
queue_BinDefs_new_list = [] ##
queue_BinDefs_list = [] ##
...
def parse_cb_csv_file(lot_file_name, old_tp, new_tp, location, lots_old_new, oldBinDef, newBinDef, site, timestamp):
    Bom = ' '
    ...
    with open(r"{}\tpi_tools\static\tpi_tools\input_files\rawdata_{}_{}_{}.csv".format(current_dir, site, location, timestamp), 'r') as data:
        ...
        if len(entry_length) > 0:
            while tracker < entry_length[0]:
                ...
                tpi_tools(queue_old_VPOs_list[0], queue_old_tpNames_list[0], queue_Sites_list[0], queue_timestamp_list[0],
                         queue_new_VPOs_list[0], queue_new_tpNames_list[0], queue_locations_list[0], queue_BinDefs_old_list[0], queue_BinDefs_new_list[0])
                ...
                queue_BinDefs_old_list.pop(0)
                queue_BinDefs_new_list.pop(0)
...
def populateQueues(old_VPOs, new_VPOs, old_TP_Name, new_TP_Name, old_TP_Path, new_TP_Path, locations, Sites):
    try:
        ...
        print(oldTP_Path, newTP_Path)
        # binDef_Path, current_time = binDefPath(newTP_Path)
        binDef_Path_old, current_time = binDefPath(oldTP_Path, 1)
        binDef_Path_new, current_time = binDefPath(newTP_Path, 2)

        username = 'myunggeu' + '_' + str(len(queue_old_VPOs_list))
        #was vabarca
        entry_length_num = 0
        # steps = locations.split(",")
        # for step in steps:
        print("THESE ARE THE LOCATIONS --> ", locations)
        # new_locations = locations.remove(None)
        for step in locations:
            print("THIS IS THE STEP---> ", step)
            if step != None:
                print("THE STEP WENT THROUGH --> ", step)
                entry_length_num += 1
                ...
                queue_BinDefs_old_list.append(binDef_Path_old)
                queue_BinDefs_new_list.append(binDef_Path_new)
...
def binDefPath(TP, old_new):
    ...
    if old_new == 1:
        binDefFile = "BinDef_file_old_" + str(current_time) + ".txt"
    elif old_new == 2:
        binDefFile = "BinDef_file_new_" + str(current_time) + ".txt"

    binDefPath = "C:/Users/myunggeu/VPO_TEMPLATES/BinDef/" + binDefFile
...
def tpi_tools(old_vpo, old_tp, site, timestamp, new_vpo, new_tp, location, oldBinDef, newBinDef):
    ...
    execute_cb_script(lot_file_name, location, site, timestamp)
    parse_cb_csv_file(lot_file_name, old_tp, new_tp, location, lots_old_new, oldBinDef, newBinDef, site, timestamp)
    ###############################################################################
...
def create_cb_script(lots, sample_cb_script, location, lot_file_name, lab, timestamp):
    ...
    cb_file = open(r'{}\tpi_tools\static\tpi_tools\input_files\raw_data_extract_{}_{}_{}.acs'.format(current_dir, lab, location, timestamp), 'w')

    cb_file.write("<!--\n\n")
    cb_file.write(" User: myunggeu\n")
    ...
    for lotname in lots:
        ...
        cb_file.write("    {} SITE=MIDAS DOMAIN=CLASS\n".format(lotname))
    ...
    for key in params.keys():
        ...
        if 'FIELD' in params[key].keys():
            cb_file.write(" <param DOMAIN={} SITE=MIDAS OPERATION={} DATATYPE={} FIELD={} TABLE={} />\n"
            ...
            cb_file.write(" <param DOMAIN={} SITE=MIDAS OPERATION={} DATATYPE={} TESTNAME={} />\n"
                            .format(params[key]['DOMAIN'],
                                    # site,
                                    location,
                                    params[key]['DATATYPE'],
    ...
    cb_file.write(
        "  /OUTPUT='{}\\tpi_tools\\static\\tpi_tools\\input_files\\rawdata_{}_{}_{}.csv'\n".format(current_dir, lab, location, timestamp))
...
def execute_cb_script(file_name, location, site, timestamp):
    print("INSIDE execute_cb_script FUNCTION")
    if os.path.isfile(r"{}\tpi_tools\static\tpi_tools\input_files\raw_data_extract_{}_{}_{}.csv".format(current_dir, site, location, timestamp)):
        os.remove(r"{}\tpi_tools\static\tpi_tools\input_files\raw_data_extract_{}_{}_{}.acs".format(current_dir, site, location, timestamp))
    if os.path.isfile(r"{}\tpi_tools\static\tpi_tools\input_files\rawdata_{}_{}_{}.csv".format(current_dir, site, location, timestamp)):
        os.remove(r"{}\tpi_tools\static\tpi_tools\input_files\rawdata_{}_{}_{}.csv".format(current_dir, site, location, timestamp))
    os.system(r"{}\tpi_tools\static\tpi_tools\input_files\raw_data_extract_{}_{}_{}.acs".format(current_dir, site, location, timestamp))
    while os.path.isfile(r"{}\tpi_tools\static\tpi_tools\input_files\rawdata_{}_{}_{}.csv".format(current_dir, site, location, timestamp)) == False:
        time.sleep(2)
    time.sleep(35)
...
def contact(request):
    hostName = index(request)
    template = loader.get_template('tpi_tools/contact.html')
    context = {
        'hostName': hostName,
    }
    return HttpResponse(template.render(context, request))
...
def about(request):
    hostName = index(request)
    template = loader.get_template('tpi_tools/about.html')
    context = {
        'hostName': hostName,
    }
    return HttpResponse(template.render(context, request))
